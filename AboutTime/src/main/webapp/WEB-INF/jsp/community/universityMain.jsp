		...
		<div class="container">
			<div class="row">
					...
					<div id="schedule-input" class="hidden content-box">
						<form>
							<div class="form-group">
								<label for="calendar-title">제목</label> <input type="text"
									class="form-control" id="calendar-title">
							</div>
							<div class="form-group">
								<label for="calendar-contents">내용</label>
								<textarea rows="3" class="form-control" id="calendar-contents"
								...
								<div class="col-xs-6">
									<div class="form-group">
										<label for="startDate">시작일</label> <input type="date"
											class="form-control" id="startDate" readonly>
									</div>
								</div>
								<div class="col-xs-6">
									<div class="form-group">
										<label for="endDate">종료일</label> <input type="date"
											class="form-control" id="endDate" readonly>
									</div>
								</div>
							</div>
				...
				<div class="col-md-4 col-md-offset-1 col-sm-6 col-xs-12">
					<div class="headline">
						<a href="/community/${university}/list">Weekly Best</a>
					</div>
					<div>
						<table class="table table-condensed">
							<c:forEach var="board" items="${dailyBoards}">
								<tr>
									<td><a href="/community/${university}/read/${board.num}">${board.title}<c:if
												test="${board.commentNum != 0}">
												<span style="font-weight: bold;">
													(${board.commentNum})</span>
											</c:if></a></td>
									<td>
										<c:choose>
											<c:when test="${board.category == '유머'}">
												<a class="sub" href="/community/${university}/list?cate=humor">${board.category}</a>
											</c:when>
											<c:when test="${board.category == '정보'}">
												<a class="sub" href="/community/${university}/list?cate=info">${board.category}</a>
											</c:when>
											<c:when test="${board.category == '잡담'}">
												<a class="sub" href="/community/${university}/list?cate=other">${board.category}</a>
											</c:when>
											<c:when test="${board.category == '질문'}">
												<a class="sub" href="/community/${university}/list?cate=quest">${board.category}</a>
											</c:when>
										</c:choose>
									</td>
									<td><p class="sub">${board.writer}</p></td>
								</tr>
							</c:forEach>
						</table>
					</div>
			...
			<div class="row">
				<div class="col-md-4 col-sm-6 col-xs-12">
					<div class="headline">
						<a href="/community/${university}/list?cate=info">정보 게시판</a>
					</div>
					<div>
						<table class="table table-condensed">
				...
				<div class="col-md-4 col-md-offset-3 col-sm-6 col-xs-12">
					<div class="headline">
						<a href="/community/${university}/list?cate=other">자유 게시판</a>
					</div>
					<div>
						<table class="table table-condensed">
			...
			<div class="row">
				<div class="col-md-4 col-sm-6 col-xs-12">
					<div class="headline">
						<a href="/community/${university}/list?cate=humor">유머 게시판</a>
					</div>
					<div>
						<table class="table table-condensed">
				...
				<div class="col-md-4 col-md-offset-3 col-sm-6 col-xs-12">
					<div class="headline">
						<a href="/community/${university}/list?cate=quest">질문 게시판</a>
					</div>
					<div>
						<table class="table table-condensed">
